...
def best_letters_index(letter_dictionary, m_letters):
    ...
    best_m_index = heapq.nlargest(m_letters, range(len(letter_freq)), letter_freq.__getitem__)
...
def best_words (partial_word_list, word_frequency, n):
    import heapq
    best_n_index = heapq.nlargest(n, range(len(word_frequency)), word_frequency.__getitem__) #Gets indices for largest n frequency values
...
def frequecy_analysis(remaining_word_list, green_yellow, no_letters):
    ...
    best_ltr_scores = best_letter_scores(best_letter_index[0], best_letter_index[2])
